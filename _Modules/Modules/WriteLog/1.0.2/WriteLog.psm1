...
#				   ,ParameterSetName='PStartLineId'
		...
		[Alias('sln')]
		[int]$StartLineNum		# начальный порядковый номер сообщения
...
#		[ValidateScript({ ($_ -gt 0) })]               
		[Alias('t')]
		[int]$Type		# Тип сообщения - числовое значение
...
#				   ,ParameterSetName='One'
	...
	# ---------------------------------------------------------------
	# если конкретные переключатели не установлены, восстановим их из ХТ
	...
	if (!([System.IO.FileInfo]$LogName).Exists -or (Get-Item $LogName).Length -eq 0) {
		Set-Content -Path $LogName -Value $Hdr 
	}
