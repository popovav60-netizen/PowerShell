...
class ABSPath {
	...
	# 18.01.2026
	# подстановочние символы [] для имени папки PowerShell
	static [string]$PatternSubstFolder 			= '\[[^\\]*\]'
...
class TxtPlain {
	...
	static [string]$Ext	='ba[st]|c(er|l[as]?|o([bd]|nfig)|pp|fg|rt|s[sv])?|d(bt|os)?|e(mf|rr|sh|xc)|f(ft|qy|rm)|h(h|hc|tml?)?|ini|j(s(on)?|w)|l(gc|og)|nuspec|p(fx|hp|ls?|pl|s(1xml|m?1|d1|t))|py|rtf|s(e|no)|txt|v(b[as]|ls)|w(bt|lg)|x(ml|sd|slt?|y)'
