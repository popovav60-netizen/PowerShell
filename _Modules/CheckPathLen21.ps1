...
Set-Variable -Name ModuleConst 		-Value "CommonConst" 							-Option Constant -Scope Script -Visibility Private
	...
	if (Check-BlankValue $PathCheck) {
		if ($env:COMPUTERNAME -eq "POPOVAV-2-0014") {
			$PathCheck = Get-RegValue $RegKey 'PathCheck' 'k:\WORK_PC\PROJECT\EXCEL\L\l6i159l1'
		}
		else {
...
if ($IsDebug -eq $true) { 
	...
	$CSVFileScan = [system.io.path]::GetTempPath() + $ScanFolderName + '.csv'
...
write-verbose "`$MsgMap $MsgMap"	
	...
	$NewLine
	Write-Host "MS Excel с информацией загружен"  -ForegroundColor Green
