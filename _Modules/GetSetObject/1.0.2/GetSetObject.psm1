...
# формируем имя файла для сохранения глобальной хэш таблицы
...
# и при наличии ранее сохраненных данных в файле, 
# восстанавливаем хэш таблицу
function script:m_sbGet-SavedHash( $pCaller ) {
	...
	if (([System.IO.FileInfo]$CLI).Exists) {
...
function script:m_sbSave-Hash( $pCaller ) {
...
<# 
	.SYNOPSIS
		Сохраняет значение в глобальной хеш таблице
	.DESCRIPTION
		Если глобальная хеш таблица не существует, она создается и туда записывается значение объекта
	...
	.PARAMETER BoxName
		Имя, под которым сохраняем значение объекта
	.PARAMETER ValueObject
		Значение объекта которое сохраняем
	.EXAMPLE
...
#>
	...
	m_sbGet-SavedHash($Caller)

	# сохраняем значение объекта в таблице
	$global:GV_GlobalHashBox.$BoxName = $ValueObject
	
	# сохраняем хэш таблицу
	m_sbSave-Hash($Caller)
...
<# 
	.SYNOPSIS
		Возвращает значение из глобальной хеш таблицы
	.DESCRIPTION
		...
		файла, который привязывается к вызывающему объекту
...
#>
	...
	# если нет сохраненного значения 
